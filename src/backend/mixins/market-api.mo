...
mixin (
  ...
  func parseNewsItems(body : Text) : [Types.NewsItem] {
    // CoinGecko /news response: { "data": [ { "title": ..., "news_site": ..., "url": ..., "published_at": ... }, ... ] }
    let after = switch (sliceAfter(body, "\"data\":")) {
      case null { body };
      case (?s) { s };
    };
    let trimmed = switch (after.trim(#text " ").stripStart(#char '[')) {
      case (?t) t;
      case null after;
    };
    let buf = List.empty<Types.NewsItem>();
    let chunks = trimmed.split(#text "},{");
    for (chunk in chunks) {
      let title = extractStr(chunk, "title");
      if (title != "") {
        let source = extractStr(chunk, "news_site");
        let url = extractStr(chunk, "url");
        let publishedAt = switch (sliceAfter(chunk, "\"published_at\":")) {
          case null 0;
          case (?s) {
            let secs = floatToNatSafe(parseFloatFromIter(s.toIter()));
            secs * 1_000_000_000;
          };
        };
        buf.add({ title; source; url; publishedAt });
      };
    };
    buf.toArray();
  };

  // Returns latest news items cached per-coinId for 5 minutes.
  // Pass "" or "general" for global crypto news.
  public func getCoinNews(coinId : Text) : async Common.ApiResult<[Types.NewsItem]> {
    let key = if (coinId == "") "general" else coinId;
    let ttl : Int = 300_000_000_000;
    let now = now_();
    if (MarketLib.isNewsFresh(newsCache, key, ttl, now)) {
      return MarketLib.buildNewsResponse(newsCache, key);
    };
    let url = "https://api.coingecko.com/api/v3/news?per_page=5";
    try {
      let body = await OutCall.httpGetRequest(url, [], transform);
      let items = parseNewsItems(body);
      MarketLib.updateNewsCache(newsCache, key, items, now);
      MarketLib.buildNewsResponse(newsCache, key);
    } catch (e) {
      switch (MarketLib.buildNewsResponse(newsCache, key)) {
        case (#ok r) { #ok r };
        case (#err _) { #err ("HTTP outcall failed: " # e.message()) };
      };
    };
