...
function TableHeader(props: {
  ...
  const { t } = props;
  return (
    <div className="grid grid-cols-[auto_auto_1fr_auto_auto] sm:grid-cols-[auto_auto_1fr_auto_auto_auto_auto] md:grid-cols-[auto_auto_1fr_auto_auto_auto_auto_auto] items-center gap-2 sm:gap-4 px-3 sm:px-4 py-2.5 border-b border-border/60 bg-card/95 backdrop-blur-sm sticky top-14 z-30 rounded-t-xl">
      <span className="w-6" />
      ...
      <Col
        k="price"
        label={t("market.col.price")}
        className="justify-end"
        {...props}
      />
      <Col
        k="change"
        label={`${props.timeframe} %`}
        className="justify-end min-w-[60px]"
        {...props}
      />
      <Col
        k="volume"
        label={t("market.col.volume")}
        className="hidden md:flex justify-end min-w-[80px]"
        {...props}
      />
      <Col
        k="marketCap"
        label={t("market.col.marketcap")}
        className="hidden sm:flex justify-end min-w-[88px]"
        {...props}
      />
    </div>
  );
...
export default function MarketPage() {
  ...
  const currencyKey = `${settings.currency}:${settings.language}`;
  const flashes = usePriceDirections(coins);

  // When logged out, tapping the star prompts login instead of silently failing.
  const handleToggleFavorite = useCallback(
    ...
    [watchlist.isAuthenticated, watchlist.toggle, login],
  );
  ...
  useEffect(() => {
    if (dataUpdatedAt) {
      ...
      lastSuccessAtRef.current = dataUpdatedAt;
    }
  }, [dataUpdatedAt]);
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      if (sortKey === "rank") diff = a.marketCapRank - b.marketCapRank;
        ...
        diff = pctFor(a, timeframe) - pctFor(b, timeframe);
  ...
  const counts = useMemo(() => {
    if (coins.length === 0)
      return { gainers: 0, losers: 0, watchlist: 0, all: 0 };
    let g = 0;
    let l = 0;
  ...
  return (
    <Layout
      ...
      isLive={
        ...
        Date.now() - lastSuccessAtRef.current < LIVE_THRESHOLD_MS
      }
      isLoading={isLoading}
    >
      <GlobalStatsBar
      ...
      {/* Tabs */}
      <div
        className="flex items-center gap-1 mb-3 overflow-x-auto -mx-1 px-1"
        data-ocid="market.tabs"
      >
        {(
          [
            { key: "all", label: t("market.tab.all"), count: counts.all },
            {
              key: "watchlist",
              label: t("market.tab.watchlist"),
              count: counts.watchlist,
              icon: <StarIcon className="w-3 h-3" />,
            },
            {
              key: "gainers",
              label: t("market.tab.gainers"),
              count: counts.gainers,
            },
            {
              key: "losers",
              label: t("market.tab.losers"),
              count: counts.losers,
            },
          ] as Array<{
            key: ViewTab;
      ...
      {/* Table */}
      <div className="rounded-xl border border-border/60 bg-background shadow-subtle">
        <TableHeader
          sortKey={sortKey}
          sortDir={sortDir}
          timeframe={timeframe}
        ...
        {!isLoading &&
          ...
          !watchlist.isAuthenticated && (
            <LoginPrompt
              titleKey="login.watchlistTitle"
              descKey="login.watchlistDesc"
            />
          )}

        {!isLoading &&
          !isError &&
          !(tab === "watchlist" && !watchlist.isAuthenticated) &&
          filteredAndSorted.length === 0 && (
            <div
              className="flex flex-col items-center justify-center gap-2 py-16 text-center px-6"
            ...
            >
              {tab === "watchlist" && watchlist.ids.length === 0 ? (
                ...
                <>
                  <p className="text-muted-foreground text-sm">
                    {t("market.noResults", { query: search })}
                  </p>
                  <button
                    type="button"
                    onClick={() => setSearch("")}
      ...
      {!isLoading && !isError && canAutoLoad && hasNextPage && (
        <div className="flex justify-center mt-4">
          ...
          >
            {isFetchingNextPage
              ? `${t("market.loadMore")} (${coins.length} / ${TOTAL_COINS_TARGET})`
              : `${t("market.loadMore")} (${coins.length} / ${TOTAL_COINS_TARGET})`}
          </button>
        </div>
      )}
      ...
      {!isLoading &&
        ...
        canAutoLoad &&
        !hasNextPage &&
        coins.length >= MAX_PAGES * 100 && (
          <p className="text-center text-xs text-muted-foreground mt-4">
            {t("market.allLoaded", { count: coins.length })}
          </p>
        )}
